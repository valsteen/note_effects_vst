...
use {
    crate::ipc_worker::{spawn_ipc_worker, IPCWorkerCommand},
...
impl ArpegiatorPatternReceiver {
    ...
    fn stop_worker(&mut self) {
        if let Some(sender) = take(&mut self.ipc_worker_sender) {
            sender
                .try_send(IPCWorkerCommand::Stop)
                .unwrap_or_else(|err| error!("Error while closing sender channel : {}", err));
            sender.close();
        }
    }
}
...
impl Plugin for ArpegiatorPatternReceiver {
    ...
    fn resume(&mut self) {
        ...
        #[cfg(not(feature = "midi_hack_transmission"))]
        {
            ...
            sender
                .try_send(IPCWorkerCommand::SetPort(self.parameters.get_port()))
                .unwrap();
