...
impl PartialOrd for SourceChange {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        match self {
            SourceChange::PatternChange(pattern) => match other {
                SourceChange::PatternChange(other_pattern) => pattern.partial_cmp(other_pattern),
                SourceChange::NoteChange(_) => Option::Some(Ordering::Greater),
            },
            SourceChange::NoteChange(note) => match other {
                SourceChange::PatternChange(_) => Option::Some(Ordering::Less),
                SourceChange::NoteChange(other_note) => note.partial_cmp(other_note),
            },
        }
    }
}
