...
impl PatternDevice {
    pub fn update(&mut self, change: DeviceChange) -> PatternDeviceChange {
        match change {
            DeviceChange::AddNote { time, note } => {
                ...
                match pattern {
                    None => PatternDeviceChange::AddPattern {
                        time,
                        pattern: new_pattern,
                    },
                    Some(old_pattern) => PatternDeviceChange::ReplacePattern {
                        time,
                        old_pattern,
                        new_pattern,
                    },
            ...
            DeviceChange::RemoveNote { time, note } => match self.patterns.remove(&note.id) {
            ...
            DeviceChange::NoteExpressionChange { time, expression, note } => match self.patterns.get_mut(&note.id) {
                None => PatternDeviceChange::None { time },
                Some(pattern) => {
    ...
    pub fn at(&self, index: u8) -> Filter<Values<'_, usize, Pattern>, PatternIteratorClosure> {
        self.patterns
            .values()
            .filter(Box::new(move |pattern| pattern.index == index))
    }
}
