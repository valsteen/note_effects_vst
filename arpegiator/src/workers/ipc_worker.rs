...
use util::ipc_payload::{BootstrapPayload, IPCCommand, PatternPayload};
...
async fn udp_receive_worker(socket: UdpSocket, sender: Sender<IPCWorkerCommand>) {
    ...
    while let Ok(len) = socket.recv(&mut buf).await {
        ...
        let ipc_worker_command = match bincode::deserialize::<String>(&buf[..len]) {
            Ok(ipc_server_name) => match bootstrap_ipc(ipc_server_name) {
                Ok(ipc_receiver) => {
            ...
            Err(err) => {
                error!(
                    "[{}] Ignoring invalid UDP payload ({}) - expected a name string",
                    exit_event_id, err
                );
                continue;
            }
        ...
        if let Err(err) = sender.send(ipc_worker_command).await {
            error!(
                "[{}] UDP Worker: error while trying to send worker command {}, quitting UDP Worker",
                exit_event_id, err
            );
            break;
        }
...
fn spawn_ipc_receiver_thread(
    ipc_receiver: IpcReceiver<IPCCommand>,
    ipc_worker_sender: Sender<IPCWorkerCommand>,
) -> Result<IpcSender<IPCCommand>, Box<dyn error::Error>> {
    ...
    thread::spawn(|| {
        task::block_on(async move {
                ...
                for result in results {
                    ...
                    match opaque_message.to::<IPCCommand>().unwrap() {
                        IPCCommand::PatternPayload(payload) => {
                            ...
                            if let Err(err) = ipc_worker_sender
                                .send(IPCWorkerCommand::PayloadReceived(payload, event_id))
                                .await
                            {
                        ...
                        IPCCommand::Stop(ack_channel_sender, event_id) => {
                            exit_event_id = event_id;
                            #[cfg(feature = "worker_debug")]
                            info!("[{}] Stopping IPC worker thread", exit_event_id);
            ...
            ipc_worker_sender
                .try_send(IPCWorkerCommand::IPCDisconnect(exit_event_id))
                .unwrap_or_else(|err| {
                    error!(
                        "[{}] Error {} while signaling IPC receiver quitting",
                        exit_event_id, err
                    );
                });
        })
...
pub(crate) fn spawn_ipc_worker(
    ...
    thread::spawn(move || {
        task::block_on(async move {
            ...
            let udp_worker_handle = task::spawn(udp_receive_worker(socket, ipc_worker_sender.clone()));
                ...
                match command {
                    IPCWorkerCommand::SocketReceive(ipc_receiver_from_socket, event_id) => {
                        if ipc_receiver_sender.is_some() {
                            if let Err(err) =
                                close_ipc_receiver_thread(take(&mut ipc_receiver_sender).unwrap(), event_id)
                            {
                    ...
                    IPCWorkerCommand::Stop(sender, event_id) => {
                        ...
                        match sender.send(()).await {
                            Ok(_) => {
                                info!("[{}] Quitting socket port {}", exit_event_id, port);
                            }
                            Err(err) => {
                                info!("[{}] Error while quitting socket port {}: {}", exit_event_id, port, err);
                            }
                        }
                        break;
                    }
                    ...
                    IPCWorkerCommand::PayloadReceived(payload, event_id) => {
                        ...
                        if let Err(err) = pattern_sender.send(payload).await {
                            exit_event_id = event_id;
                            error!(
                                "[{}] IPC worker: notes sender channel error, quitting ({})",
                                exit_event_id, err
                            );
                            break;
                        }
                        ...
                        #[cfg(feature = "device_debug")]
                        {
                            ...
                            info!(
                                "Received time: {:?} current time: {:?} = {} nanoseconds",
                                payload_time, local_time, diff_nanoseconds
                            );
                        }
                    }
                }
            ...
            #[cfg(feature = "worker_debug")]
            info!("[{}] stopping udp worker on port {}", exit_event_id, port);
            udp_worker_handle.cancel().await;
            #[cfg(feature = "worker_debug")]
            info!("[{}] udp worker on port {} stopped", exit_event_id, port);

            if let Some(sender) = take(&mut ipc_receiver_sender) {
                close_ipc_receiver_thread(sender, exit_event_id).unwrap_or_else(|err| {
                    info!(
                        "[{}] ipc receiver thread did not quit gracefully: {}",
                        exit_event_id, err
                    );
                })
            }

            if let Err(err) = worker_command_sender
                .send(WorkerCommand::IPCWorkerStopped(exit_event_id, port))
                .await
            {
            ...
            #[cfg(feature = "worker_debug")]
            info!("[{}] exiting ipc worker ( port {} )", exit_event_id, port);
        })
    });
...
fn close_ipc_receiver_thread(
    ipc_receiver_sender: IpcSender<IPCCommand>,
    event_id: Uuid,
) -> Result<(), Box<dyn error::Error>> {
    #[cfg(feature = "worker_debug")]
    info!("[{}] close_ipc_receiver_thread: enter", event_id);
    let (ack_sender, ack_receiver) = ipc_channel::ipc::channel::<()>()?;

    #[cfg(feature = "worker_debug")]
    info!("[{}] stopping ipc receiver", event_id);
    ipc_receiver_sender.send(IPCCommand::Stop(ack_sender, event_id))?;

    #[cfg(feature = "worker_debug")]
    info!("[{}] waiting for ack from ipc receiver", event_id);
    ack_receiver
        .try_recv()
        .map_err(|x| format!("Error while receiving ack from ipc receiver {:?}", x))?;

    #[cfg(feature = "worker_debug")]
    info!("[{}] stopped ipc receiver", event_id);
    Ok(())
}
