...
use midir::MidiInput;
#[cfg(target_os = "linux")]
use {midir::os::unix::VirtualOutput, midir::MidiOutput};
...
pub(crate) fn spawn_midi_output_worker(
    name: String,
    ...
    let mut midi_out_connection = {
        ...
        midi_out
            .create_virtual(&*name)
            .map_err(|e| format!("Cannot create midi out: {}", e))?
    };

    #[cfg(target_os = "macos")]
    let (client, source) = {
        let client = coremidi::Client::new(&*name).map_err(|x| format!("os error: {:?}", x))?;
        let source = client
            .virtual_source(&*name)
            .map_err(|x| format!("os error: {:?}", x))?;
    ...
    task::spawn(async move {
        ...
        while let Ok(command) = receiver.recv().await {
            match command {
                ...
                MidiOutputWorkerCommand::SendToController {
                    reception_time,
                    mut messages,
                } => {
                    #[cfg(target_os = "linux")]
                ...
                MidiOutputWorkerCommand::Stop(sender, event_id) => {
                    match sender.send(()).await {
                        Ok(_) => {
                            info!("[{}] Stopping controller {}", event_id, name);
                        }
                        Err(err) => {
                            info!("[{}] Error while quitting midi out {}: {}", event_id, name, err);
                        }
                    }
                    return;
                }
